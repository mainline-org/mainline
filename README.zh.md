...
你不用记这些 — `mainline init` 会安装一个 Mainline skill，给 agent 完整的
协议说明。如果 hooks 已安装（init 也会默认做），agent 在每次会话开始时
自动收到最新的团队状态。
...
带 **[you]** 的行是你输入的；其他行是 agent 跑的（受 Mainline skill
驱动，或通过 hooks 自动注入）。
...
# hooks 已由 init 默认安装；如需修复或手动添加特定 agent：
