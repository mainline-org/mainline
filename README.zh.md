...
你不用记这些。已安装的 `mainline` agent skill 是首选的完整 workflow
source of truth。仓库内的 `AGENTS.md` / IDE guidance 只保留轻量 marker 和
bootstrap 提醒，给没有加载 skill 的 agent 兜底。
...
check），这由 `mainline` skill 或团队 guidance 规定 — Mainline 是
context provider，不是 workflow driver。

如果你不用支持的 hook agent，你的 AI 工具按已安装的 `mainline` skill
执行；仓库内 `AGENTS.md` 只作为轻量 fallback marker 和 bootstrap 提醒。
...
带 **[you]** 的行是你输入的；其他行是 agent 跑的（受 `mainline`
skill / team guidance 驱动，或装了 hooks 后自动注入 context）。
