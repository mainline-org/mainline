...
// call described by the Mainline skill or team guidance.
type EngineFacade interface {
	...
	// whether to start / append / seal — exactly as the Mainline skill
	// or team guidance instructs it to in the no-hook flow.
	Status() (any, error)
...
func (d *Dispatcher) Dispatch(ctx context.Context, ev *Event) error {
	...
	case TurnStart:
		...
		// agent's job per the Mainline skill or team guidance.
		d.Bus.Emit(d.envelope("turn_started", ev, nil))
		return nil
	case TurnEnd, SubagentEnd:
		// Webhook-only signal. Hooks cannot judge whether the
		// turn warrants a mainline append — that's the agent's
		// job per the Mainline skill or team guidance ("after each meaningful logical
...
//   - a scenario hint that points the agent at the Mainline skill or
//     repo-local AGENTS.md bootstrap marker, plus the CLI commands it
//     should run before deciding (`mainline log` and `mainline show`).
//
...
func (d *Dispatcher) RenderSessionStartContext(syncResult any, status any) string {
	...
	b.WriteString("Use this snapshot to orient yourself. The full workflow contract lives in the `mainline` skill or team guidance; ")
	b.WriteString("repo-local AGENTS.md is only a bootstrap marker. Hooks do not replace any agent judgment.\n\n")
	...
	b.WriteString("Follow the Mainline skill or team guidance exactly — hooks change nothing about the workflow:\n\n")
	...
	b.WriteString("- When the task is complete, commit code, then `mainline seal --prepare --json`, fill the SealResult (fingerprint generously), then `mainline seal --submit --json < seal.json`. If the response carries a `conflicts` array, surface it to the user verbatim. This does not authorize Git branch push.\n")
...
func (d *Dispatcher) RenderTurnStartContext(status any, proposals any, statusErr error, proposalsErr error) string {
	...
	b.WriteString("Use it as a reminder; the Mainline skill or team guidance remains the workflow authority.\n\n")
