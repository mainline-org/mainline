...
// IntentSummary is the structured summary of an intent.
//
// Decisions are the default seal contract. Risks, anti-patterns, and follow-ups
// are explicit-only structured signals:
//
//   - Risks are *present-review warnings* — concrete failure modes a reviewer
//     should audit. They are not future-agent rules.
//   - AntiPatterns are *explicit constraints* — high-authority rules future
//     agents must not violate. They must not be invented by a default seal.
//   - Followups are *deferred work items* — only for user-approved or clearly
//     cut scope. They are not a place for agent speculation.
//
...
// AntiPattern is an explicit hard constraint future agents MUST avoid when
// working in this intent's area. Distinct from Risk, which is a present-review
// warning. Mainline's retrieval always carries the full AntiPattern list —
// they are never truncated by top-N caps because the load-bearing safety
// property is "the agent saw the constraint before editing".
...
type SealPreparePackage struct {
	...
	// why, decisions) stay empty. Explicit-only structured signals
	// (risks, anti_patterns, followups) are deliberately omitted so
	// agents do not fill them for completeness. Optional in the
	// schema so older readers still parse the package.
